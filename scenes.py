...
_creature_surfaces: dict = {} # creature_id → Surface (raw, scaled on demand)
...
# Object sprite positions within the 1024×MAP_AREA_H scene area.
# Each entry is (cx, cy, size): centre pixel and draw size.
_OBJECT_POSITIONS = {
    "Forest":       [(160, 260, 150), (610, 360, 95),  (870, 230, 140)],
    "River":        [(330, 315, 130), (535, 390, 110), (790, 420, 90)],
    "Clearing":     [(160, 330, 110), (512, 195, 130), (800, 335, 105)],
    "Dense Jungle": [(175, 215, 145), (455, 180, 160), (800, 395, 110)],
    "Camp":         [(512, 345, 120), (800, 370, 90),  (195, 375, 90)],
}
...
def load_creature_images() -> int:
    """Load AI-generated creature/plant portrait PNGs into pygame surfaces.

    Expects files at  data/images/creatures/<creature_id>.png
    Generated by running  generate_on_gpu.py  (outputs to output_images/creatures/).
    Returns number of portraits successfully loaded.
    """
    from pathlib import Path
    creatures_dir = Path(__file__).parent / "data" / "images" / "creatures"
    if not creatures_dir.exists():
        return 0
    count = 0
    for path in sorted(creatures_dir.glob("*.png")):
        creature_id = path.stem
        try:
            raw = pygame.image.load(str(path)).convert()
            _creature_surfaces[creature_id] = raw
            count += 1
        except Exception as e:
            print(f"[scenes] Could not load portrait {path.name}: {e}")
    if count:
        print(f"[scenes] Loaded {count} creature portraits.")
    return count
...
def _draw_object_sprite(surf, cx: int, cy: int, size: int,
                        sprite_id: str, label: str, font, hover: bool) -> None:
    """Invisible hotspot — completely see-through until hovered.

    On hover: a subtle glow ring marks the spot and a floating tooltip
    appears above it with the object name.
    """
    if not hover:
        return
    ...
    pygame.draw.circle(glow_s, (255, 255, 180, 38), (r + 2, r + 2), r)
    pygame.draw.circle(glow_s, (255, 220, 100, 180), (r + 2, r + 2), r, 2)
    surf.blit(glow_s, (cx - r - 2, cy - r - 2))

    # Floating tooltip badge above the hotspot
    clean = _EMOJI_RE.sub('', label).strip()
    img = font.render(_bidi(clean), True, C_WHITE)
    lx = cx - img.get_width() // 2
    ly = cy - size // 2 - img.get_height() - 12
    badge = Rect(lx - 10, ly - 6, img.get_width() + 20, img.get_height() + 12)
    badge_s = pygame.Surface((badge.w, badge.h), pygame.SRCALPHA)
    pygame.draw.rect(badge_s, (0, 0, 0, 200), badge_s.get_rect(), border_radius=8)
    surf.blit(badge_s, badge.topleft)
    pygame.draw.rect(surf, C_GOLD, badge, 1, border_radius=8)
    surf.blit(img, (lx, ly))
...
# ── Intro screen shared background ────────────────────────────────────────────

def _draw_intro_bg(surf, tile_type: str = "Forest") -> None:
    """Rich atmospheric background for intro/setup screens."""
    if tile_type in _scene_surfaces:
        bg = pygame.transform.smoothscale(_scene_surfaces[tile_type][0], (W, H))
        surf.blit(bg, (0, 0))
    else:
        # Gradient fallback
        for y in range(H):
            ...
            pygame.draw.line(surf,
                             (int(5 + 12 * t), int(16 + 28 * t), int(5 + 8 * t)),
                             (0, y), (W, y))
    ...
    # Subtle god-rays from upper left
    rays = pygame.Surface((W, H), pygame.SRCALPHA)
    for rx in [160, 360, 580, 790, 970]:
        pts = [(rx - 55, 0), (rx + 55, 0), (rx + 210, H), (rx - 210, H)]
        pygame.draw.polygon(rays, (190, 255, 170, 9), pts)
    surf.blit(rays, (0, 0))
...
def draw_title(surf, fonts) -> dict:
    ...
    _draw_intro_bg(surf, "Forest")

    # Decorative vine — left edge
    vine_pts = [(22, 60), (68, 170), (30, 300), (75, 430), (28, 560), (60, 680)]
    pygame.draw.lines(surf, (35, 110, 35), False, vine_pts, 4)
    for vp in vine_pts[1:-1]:
        pygame.draw.circle(surf, (48, 148, 48), vp, 10)
        pygame.draw.circle(surf, (30, 95, 30), vp, 10, 2)

    # Explorer character — lower left, looking proud
    creatures.draw_explorer(surf, 155, H - 105, size=115, frame=0, color_idx=0)

    # === Title panel ===
    panel_w, panel_h = 680, 230
    panel = Rect(W // 2 - panel_w // 2, H // 2 - panel_h // 2 - 55, panel_w, panel_h)

    # Drop shadow
    sh = pygame.Surface((panel_w + 16, panel_h + 16), pygame.SRCALPHA)
    pygame.draw.rect(sh, (0, 0, 0, 110), sh.get_rect(), border_radius=20)
    surf.blit(sh, (panel.x - 8, panel.y + 10))

    # Panel body
    pygame.draw.rect(surf, (6, 20, 6), panel, border_radius=16)
    pygame.draw.rect(surf, C_GOLD, panel, 2, border_radius=16)
    # Inner accent line
    pygame.draw.rect(surf, (48, 105, 48), panel.inflate(-12, -12), 1, border_radius=12)

    # Gold corner gems
    for px, py in [(panel.x, panel.y), (panel.right, panel.y),
                   (panel.x, panel.bottom), (panel.right, panel.bottom)]:
        pygame.draw.circle(surf, C_GOLD, (px, py), 6)
        pygame.draw.circle(surf, (255, 240, 160), (px, py), 3)

    # Title text with soft glow
    line1 = loc.t("ui.title_line1")
    line2 = loc.t("ui.title_line2")
    for dx, dy in [(-2, 0), (2, 0), (0, -2), (0, 2)]:
        draw_text(surf, line1, (W // 2 + dx, panel.y + 42 + dy),
                  fonts["xl"], (0, 70, 0), align="center")
    draw_text(surf, line1, (W // 2, panel.y + 42), fonts["xl"], C_GREEN, align="center")

    for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
        draw_text(surf, line2, (W // 2 + dx, panel.y + 112 + dy),
                  fonts["lg"], (100, 75, 0), align="center")
    draw_text(surf, line2, (W // 2, panel.y + 112), fonts["lg"], C_GOLD, align="center")

    # Separator
    sep_y = panel.y + 168
    pygame.draw.line(surf, (50, 110, 50), (panel.x + 50, sep_y), (panel.right - 50, sep_y), 1)
    draw_text(surf, "* * *", (W // 2, sep_y - 11), fonts["sm"], (70, 130, 70), align="center")

    # === Start button ===
    btn = Rect(W // 2 - 145, panel.bottom + 28, 290, 62)
    # Button glow halo
    glow = pygame.Surface((btn.w + 24, btn.h + 24), pygame.SRCALPHA)
    pygame.draw.rect(glow, (60, 200, 60, 38), glow.get_rect(), border_radius=16)
    surf.blit(glow, (btn.x - 12, btn.y - 12))
    draw_button(surf, btn, loc.t("setup.welcome"), fonts["lg"], hover=True)
...
def draw_setup_name(surf, fonts, text: str) -> dict:
    ...
    _draw_intro_bg(surf, "Camp")

    # Explorer watching from lower right
    creatures.draw_explorer(surf, W - 145, H - 105, size=115, frame=0, color_idx=0)

    # Central panel
    panel = Rect(W // 2 - 280, H // 2 - 130, 560, 260)
    sh = pygame.Surface((panel.w + 14, panel.h + 14), pygame.SRCALPHA)
    pygame.draw.rect(sh, (0, 0, 0, 100), sh.get_rect(), border_radius=18)
    surf.blit(sh, (panel.x - 7, panel.y + 8))
    pygame.draw.rect(surf, (6, 20, 6), panel, border_radius=14)
    pygame.draw.rect(surf, C_GREEN, panel, 2, border_radius=14)
    pygame.draw.rect(surf, (40, 95, 40), panel.inflate(-12, -12), 1, border_radius=10)

    draw_text(surf, loc.t("setup.ask_name"),
              (W // 2, panel.y + 30), fonts["lg"], C_GREEN, align="center")

    # Input field
    input_r = Rect(W // 2 - 210, panel.y + 92, 420, 58)
    pygame.draw.rect(surf, (18, 38, 18), input_r, border_radius=10)
    pygame.draw.rect(surf, C_GREEN, input_r, 2, border_radius=10)
    # Cursor blink — always show for simplicity
    display = text + "|"
    draw_text(surf, display, (input_r.x + 16, input_r.y + 14), fonts["lg"], C_WHITE)

    draw_text(surf, loc.t("ui.press_enter"),
              (W // 2, panel.y + 185), fonts["md"], C_DIM, align="center")
...
# Labels and colour palette index for each avatar slot
_AVATAR_LABELS = ["setup.avatar_1", "setup.avatar_2", "setup.avatar_3", "setup.avatar_4"]
_AVATAR_COLOR_IDX = [1, 2, 0, 3]   # pink, blue, khaki, green


def draw_setup_avatar(surf, fonts, hover_idx: int, selected_idx: int) -> dict:
    """Draw avatar selection screen. Returns {'btns': [Rect, ...]}."""
    _draw_intro_bg(surf, "Clearing")

    # Header
    draw_text(surf, loc.t("setup.choose_avatar"),
              (W // 2, 68), fonts["xl"], C_GREEN, align="center")
    pygame.draw.line(surf, (50, 120, 50), (W // 2 - 240, 112), (W // 2 + 240, 112), 1)

    # 4 avatar cards — sized so the explorer sprite fits with a label below
    card_w, card_h = 195, 248
    gap = 20
    ...
    for i, label_key in enumerate(_AVATAR_LABELS):
        r = Rect(start_x + i * (card_w + gap), card_top, card_w, card_h)
        hover    = (i == hover_idx)
        selected = (i == selected_idx)

        bg_col   = (45, 88, 45) if selected else (38, 68, 38) if hover else (20, 40, 20)
        edge_col = C_GOLD if selected else C_GREEN if hover else C_GREEN_DARK
        edge_w   = 3 if selected else 2

        # Card shadow
        sh = pygame.Surface((card_w + 10, card_h + 10), pygame.SRCALPHA)
        pygame.draw.rect(sh, (0, 0, 0, 80), sh.get_rect(), border_radius=14)
        surf.blit(sh, (r.x - 5, r.y + 6))

        pygame.draw.rect(surf, bg_col, r, border_radius=14)
        pygame.draw.rect(surf, edge_col, r, edge_w, border_radius=14)

        # Gold glow halo when selected
        if selected:
            gl = pygame.Surface((card_w + 20, card_h + 20), pygame.SRCALPHA)
            pygame.draw.rect(gl, (255, 215, 0, 40), gl.get_rect(), border_radius=17)
            surf.blit(gl, (r.x - 10, r.y - 10))

        # Explorer sprite — centred in upper 85% of card, label in bottom 15%
        ex_cy = r.y + int(card_h * 0.58)
        creatures.draw_explorer(surf, r.centerx, ex_cy, size=90,
                                frame=0, color_idx=_AVATAR_COLOR_IDX[i])

        lbl = loc.t(label_key)
        txt_col = C_GOLD if selected else C_WHITE if hover else C_DIM
        draw_text(surf, lbl, (r.centerx, r.bottom - 26), fonts["sm"], txt_col, align="center")

        btns.append(r)

    # Confirm button
    btn_y = card_top + card_h + 22
    if selected_idx >= 0:
        confirm = Rect(W // 2 - 135, btn_y, 270, 60)
        glow = pygame.Surface((confirm.w + 20, confirm.h + 20), pygame.SRCALPHA)
        pygame.draw.rect(glow, (60, 200, 60, 35), glow.get_rect(), border_radius=14)
        surf.blit(glow, (confirm.x - 10, confirm.y - 10))
...
def draw_scene_objects(surf, fonts, tile_type: str, objects: list,
    ...
    positions = _OBJECT_POSITIONS.get(tile_type, [])
    rects = []
    for i, (obj, pos) in enumerate(zip(objects, positions)):
        cx, cy, size = pos
        sprite_id = obj.get("sprite_id", "")
        label = loc.t(obj["name_key"])
        _draw_object_sprite(surf, cx, cy, size, sprite_id, label, fonts["md"], hover=(i == hover_idx))
        rects.append(Rect(cx - size // 2, cy - size // 2, size, size))
...
def draw_popup(surf, fonts, popup: dict) -> None:
    ...
    if kind == "discovery_new":
        ...
        if d.id in _creature_surfaces:
            portrait = pygame.transform.smoothscale(_creature_surfaces[d.id], (art_box.w, art_box.h))
            surf.blit(portrait, art_box.topleft)
        else:
...
def draw_book(surf, fonts, game_state, discoveries_db: list,
    ...
    if detail_entry:
        ...
        # Creature art — right portion (AI portrait or procedural fallback)
        art_box = Rect(panel.right - 310, panel.y + 20, 280, 280)
        pygame.draw.rect(surf, (10, 18, 30), art_box, border_radius=8)
        if detail_entry.id in _creature_surfaces:
            portrait = pygame.transform.smoothscale(_creature_surfaces[detail_entry.id], (280, 280))
            surf.blit(portrait, art_box.topleft)
        else:
            creatures.draw_creature(surf, detail_entry.id, art_box.centerx, art_box.centery, size=220)
        pygame.draw.rect(surf, C_BLUE, art_box, 2, border_radius=8)
...
def get_object_rects(tile_type: str) -> list:
    """Return the list of hotspot Rects for a tile_type (derived from sprite positions)."""
    return [Rect(cx - s // 2, cy - s // 2, s, s)
            for cx, cy, s in _OBJECT_POSITIONS.get(tile_type, [])]
